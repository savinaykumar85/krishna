		...
		return this.each(function()
		{
			...
			for (var j = 0; j < opts.markers.length; j++)
			{
				...
				if (marker.address)
				{
					...
					if (marker.php == '_address') { marker.php = marker.address; }
					...
					$geocoder.getLatLng(marker.address, function(gicon, marker)
					{
						...
						return function(gpoint)
						{
							...
							if (marker.php) { gmarker.bindInfoWindowHtml(opts.php_prepend + marker.php + opts.php_append); }
							if (marker.php && marker.popup) { gmarker.openInfoWindowHtml(opts.php_prepend + marker.php + opts.php_append); }
				...
				else
				{
					...
					if (marker.php == '_latlng') { marker.php = marker.latitude + ', ' + marker.longitude; }
